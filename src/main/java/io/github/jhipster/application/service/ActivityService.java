...
public class ActivityService {
    ...
    /**
     * Get one activity by wechatUserId.
     *
     * @param id the id of the entity
     * @return the entity
     */
    @Transactional(readOnly = true)
    public List<ActivityDTO> findOneByWechatUserId(String wechatUserId) {
    	log.debug("Request to get  Activities by wechatUserId");
        return activityRepository.findOneByWechatUserId(wechatUserId).stream()
            .map(activityMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }
